...
def downscale(image_syn, scale_factor):
    B, T, _, H, W = image_syn.shape  # _ 表示我们不要那一维
    image_syn = image_syn.view(B * T, H, W)             # 去掉 C=1，变成 [B*T, H, W]
    image_syn = image_syn.unsqueeze(1)                  # 加回通道维 → [B*T, 1, H, W]
    image_syn = F.interpolate(image_syn, scale_factor=scale_factor, mode='bilinear', align_corners=False)
    H_new, W_new = image_syn.shape[-2:]
    image_syn = image_syn.view(B, T, 1, H_new, W_new)   # 恢复原结构
    return image_syn
        ...
        for c in range(num_classes):
            if not args.idm_aug:
                ...
                for c in range(num_classes):
                    ...
                    for i in range(2):
                        for j in range(2):
                            # [B, 3, 1, H, W] → squeeze(2) → [B, 3, H, W] → downscale → [B, 3, H_s, W_s]
                            patch = downscale(auds_real[start:end].squeeze(2), 0.5).detach()

                            # 再 unsqueeze 加回第3维（channel后），变成 [B, 3, 1, H_s, W_s]
                            patch = patch.unsqueeze(2)

                            H_s, W_s = patch.shape[-2:]
                            aud_syn.data[c * args.ipc:(c + 1) * args.ipc, :,
                            :, i * H_s:(i + 1) * H_s, j * W_s:(j + 1) * W_s] = patch
                    ...
                    for i in range(2):
                        for j in range(2):
                            patch = downscale(imgs_real[start:end], 0.5).detach()
                            H_s, W_s = patch.shape[-2:]
