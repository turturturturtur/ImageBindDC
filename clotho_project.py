...
def visualize_similarity_matrices(real_audio: torch.Tensor, 
                                  real_text: torch.Tensor, 
                                  distilled_results: dict, 
                                  file_path_prefix: str):
    """
    使用热力图可视化并对比真实数据与合成数据的音-文交叉相似度矩阵。

    Args:
        real_audio (torch.Tensor): 真实的音频特征张量。
        real_text (torch.Tensor): 真实的文本特征张量。
        distilled_results (dict): 一个字典，键为方法名 (如 'AVDD_ImageBind')，
                                  值为包含 'synthetic_audio' 和 'synthetic_text' 的字典。
        file_path_prefix (str): 保存输出图片的文件路径前缀 (例如 'results/ipc20_seed42')。
    """
    print(f"\n🔥 正在生成相似度矩阵热力图...")

    # 1. 计算真实数据的相似度矩阵作为基准 (Ground Truth)
    #    在计算余弦相似度前，对特征进行L2归一化
    real_audio_norm = F.normalize(real_audio)
    real_text_norm = F.normalize(real_text)
    real_sim = torch.matmul(real_audio_norm, real_text_norm.t()).cpu().numpy()

    # 2. 根据蒸馏方法的数量，创建合适数量的并列子图
    num_methods = len(distilled_results)
    fig, axes = plt.subplots(1, num_methods + 1, figsize=(6 * (num_methods + 1), 5.5))
    fig.suptitle('Comparison of Cross-Modal Similarity Matrices', fontsize=20, y=1.0)

    # 3. 绘制左侧的基准图 (真实数据)
    sns.heatmap(real_sim, ax=axes[0], cmap='viridis', cbar=True)
    axes[0].set_title('Ground Truth (Real Data)', fontsize=16)
    axes[0].set_xlabel('Text Features', fontsize=12)
    axes[0].set_ylabel('Audio Features', fontsize=12)
    axes[0].set_xticks([])
    axes[0].set_yticks([])

    # 4. 迭代绘制每一种蒸馏方法产出的合成数据的热力图
    method_list = list(distilled_results.items())
    for i in range(num_methods):
        method_name, results = method_list[i]
        syn_audio = results['synthetic_audio']
        syn_text = results['synthetic_text']
        
        # 计算合成数据的相似度矩阵
        syn_audio_norm = F.normalize(syn_audio)
        syn_text_norm = F.normalize(syn_text)
        syn_sim = torch.matmul(syn_audio_norm, syn_text_norm.t()).cpu().numpy()
        
        # 绘制热力图，为保持整体简洁，只为基准图添加颜色条(colorbar)
        sns.heatmap(syn_sim, ax=axes[i+1], cmap='viridis', cbar=False)
        axes[i+1].set_title(f'Synthetic ({method_name})', fontsize=16)
        axes[i+1].set_xlabel('Text Features', fontsize=12)
        axes[i+1].set_ylabel('')  # 隐藏Y轴标签以避免重复
        axes[i+1].set_xticks([])
        axes[i+1].set_yticks([])
    ...
    # 5. 保存最终的对比图
    output_path = f"{file_path_prefix}_similarity_matrix.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✅ 相似度矩阵热力图已成功保存至: {output_path}")

def visualize_tsne_comparison(
    results_list, # <-- 【关键修改】接收一个包含所有方法结果的列表
    file_path,
    num_samples=500
):
    """
    使用t-SNE【对比性地】可视化多个方法的音文联合嵌入空间，并将大图保存到文件。
    """
    print(f"\n🎨 正在生成【对比性】的t-SNE嵌入空间图 (采样数: {num_samples})，并保存至 {file_path}...")

    # 1. 准备画布：创建一个1行3列的子图布局
    num_methods = len(results_list)
    fig, axes = plt.subplots(1, num_methods, figsize=(10 * num_methods, 8)) # 调整画布大小
    fig.suptitle('t-SNE Visualization of Audio-Text Joint Embedding Space', fontsize=24, y=1.02)

    all_embeddings_2d = []
    
    # 2. 为所有方法的数据统一运行t-SNE，以确保坐标系可比
    all_features_stacked = []
    all_labels = []
    method_indices = []
    
    for result in results_list:
        audio_features = result['projected_audio']
        text_features = result['projected_text']
        
        # 随机采样
        if len(audio_features) > num_samples:
            indices = np.random.choice(len(audio_features), num_samples, replace=False)
            audio_features = audio_features[indices]
            text_features = text_features[indices]

        all_features_stacked.append(audio_features.cpu().numpy())
        all_features_stacked.append(text_features.cpu().numpy())
        all_labels.extend(['Audio'] * len(audio_features))
        all_labels.extend(['Text'] * len(text_features))
        method_indices.extend([result['name']] * (len(audio_features) + len(text_features)))

    print("  正在运行t-SNE降维...")
    tsne = TSNE(n_components=2, perplexity=30, max_iter=1000, random_state=42)
    embeddings_2d = tsne.fit_transform(np.vstack(all_features_stacked))
    ...
    for i, result in enumerate(results_list):
        ...
        sns.scatterplot(
            ...
            y='y', 
            hue='modality',
            palette={'Audio': 'skyblue', 'Text': 'coral'},
            alpha=0.8,
            ax=ax,
            legend= (i == num_methods - 1) # 只在最后一个子图上显示图例
        )
        
        # 设置子图标题和更大的字体
        ax.set_title(f'({chr(97+i)}) {method_name}', fontsize=20)
...
class ClothoExperiment:
    ...
    def run_complete_experiment(self, args):
        ...
        # ===================== 阶段二：蒸馏结果可视化 =====================
        print("\n" + "=" * 80)
        print("🖼️  阶段二：开始可视化蒸馏结果 -> 相似度矩阵对比")
        print("=" * 80)

        # 1. 将所有蒸馏方法的结果打包到一个字典中
        ...
        # 2. 构造用于保存可视化结果的文件名前缀。
        #    文件将直接保存在脚本运行的【当前目录】下。
        file_prefix = f'ipc{args.ipc}_seed{args.seed}'
        print(f"可视化结果将以 '{file_prefix}_similarity_matrix.png' 的格式保存在当前目录。")

        # 3. 调用可视化函数
        visualize_similarity_matrices(
            real_audio=real_imagebind_audio,
            real_text=real_imagebind_text,
            distilled_results=all_distilled_results,
