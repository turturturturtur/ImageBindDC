...
from utils.train_utils_DM import evaluate_synset_av, get_network, get_network_imagebind
...
def downscale(image_syn, scale_factor):
    target_h = int(image_syn.shape[-2] * scale_factor)
    target_w = int(image_syn.shape[-1] * scale_factor)
    image_syn = F.upsample(image_syn, size=(target_h, target_w), mode='bilinear')
    return image_syn
    ...
    for exp in range(args.num_exp):
        ...
        def get_aud_images_init(c, n): 
            if args.init_herding and args.dataset=='Music_21':        
                ...
                # 在 get_aud_images_init 函数中，确保音频数据是5D的
        ...
        for c in range(num_classes):
            if not args.idm_aug:
                ...
                for c in range(num_classes):
                    ...
                    # Fix: Ensure correct tensor dimensions for audio
                    aud_chunk = auds_real[start:end].squeeze(2)  # Remove extra dimension
                    if aud_chunk.dim() == 3:  # (batch, H, W)
                        aud_chunk = aud_chunk.unsqueeze(1)  # Add channel dim: (batch, 1, H, W)
                    
                    # Use exact size instead of downscale to avoid rounding issues
                    aud_down = F.interpolate(aud_chunk, size=(a_half_h, a_half_w), mode='bilinear', align_corners=False)
                    aud_syn.data[c*args.ipc:(c+1)*args.ipc, :, :, :a_half_h, :a_half_w] = aud_down.data.unsqueeze(2)
                    
                    start,end = end, end+args.ipc
                    aud_chunk = auds_real[start:end].squeeze(2)
                    if aud_chunk.dim() == 3:
                        aud_chunk = aud_chunk.unsqueeze(1)
                    aud_down = F.interpolate(aud_chunk, size=(a_half_h, a_half_w), mode='bilinear', align_corners=False)
                    aud_syn.data[c*args.ipc:(c+1)*args.ipc, :, :, a_half_h:, :a_half_w] = aud_down.data.unsqueeze(2)
                    ...
                    # Fix: Ensure correct tensor dimensions for images
                    img_chunk = imgs_real[start:end]
                    if img_chunk.dim() == 3:  # (batch, H, W)
                        img_chunk = img_chunk.unsqueeze(1)  # Add channel dim: (batch, 1, H, W)
                    
                    img_down = F.interpolate(img_chunk, size=(v_half_size, v_half_size), mode='bilinear', align_corners=False)
                    image_syn.data[c*args.ipc:(c+1)*args.ipc, :, :v_half_size, :v_half_size] = img_down.data.squeeze(1)
        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            for c in range(num_classes):
                ...
                if args.input_modality == 'av':                    
                    ...
                    # loss_c += args.lam_icm*match_loss(embd_aud_real+embd_img_real, embd_aud_syn+embd_img_syn, args)
                    ...
                    # 逐元素乘法
                    real_combined = embd_aud_real_norm * embd_img_real_norm  # (128, 1024)
                    syn_combined = embd_aud_syn_norm * embd_img_syn_norm  # (4, 1024)

                    # 计算余弦相似度
                    cos_sim = torch.mm(real_combined, syn_combined.T)  # 结果是 (128, 4)

                    # print(f"Implicit Cross Matching cos_sim:{cos_sim}")
                    ...
                    # loss_c += args.lam_cgm*match_loss(torch.mean(embd_aud_real, dim=0) + torch.mean(embd_img_syn, dim=0), torch.mean(embd_img_real, dim=0) + torch.mean(embd_aud_syn, dim=0), args)

                    # 计算平均值（每个维度的均值）
                    aud_real_avg = torch.mean(embd_aud_real, dim=0)  # (1024,)
                    img_syn_avg = torch.mean(embd_img_syn, dim=0)    # (1024,)
                    img_real_avg = torch.mean(embd_img_real, dim=0)  # (1024,)
                    aud_syn_avg = torch.mean(embd_aud_syn, dim=0)    # (1024,)

                    # 转换为二维矩阵进行矩阵乘法
                    aud_real_avg = aud_real_avg.unsqueeze(0)  # (1, 1024)
                    img_syn_avg = img_syn_avg.unsqueeze(0)    # (1, 1024)
                    img_real_avg = img_real_avg.unsqueeze(0)  # (1, 1024)
                    aud_syn_avg = aud_syn_avg.unsqueeze(0)    # (1, 1024)

                    cross_cos_lsss = torch.mm(aud_real_avg * img_syn_avg, (img_real_avg * aud_syn_avg).T)
                    # print(f"Cross-Modal Gap Matching cross_cos_lsss:{cross_cos_lsss}")
