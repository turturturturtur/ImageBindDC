...
class PretrainedConvNetExtractor(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_dim: int = 256, feature_dim: int = 512, device='cuda'):
        ...
        class MockConvNetArgs:
            def __init__(self):
                self.arch_sound = 'convNet' 
                self.weights_sound = ''     
                self.arch_frame = 'convNet' 
                self.weights_frame = ''     
                self.arch_classifier = 'ensemble' 
                self.cls_num = 10 
                self.weights_classifier = ''
                self.input_modality = 'a' 
        ...
        # 【关键修正】将线性层的输入维度从 512 修改为 6272
        self.audio_projector = nn.Linear(6272, feature_dim)
...
# 在 PretrainedConvNetExtractor 类中
    def forward_text(self, text: torch.Tensor) -> torch.Tensor:
        # 【关键修正】添加 torch.clamp 防止索引越界
