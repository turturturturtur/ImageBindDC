    ...
    for exp in range(args.num_exp):
        ...
        def get_aud_images_init(c, n): 
            if args.init_herding and args.dataset=='Music_21':        
                ...
                if args.input_modality == 'a' or args.input_modality == 'av':
                    ...
                    # Ensure audio dimensions are consistent
                    if idx_aud is not None:
        ...
        for c in range(num_classes):
            if not args.idm_aug:
                ...
                if args.input_modality == 'a' or args.input_modality == 'av':
                    # Ensure audio data has proper dimensions
                    if aud_real_init is not None:
                        print(f"DEBUG: aud_real_init shape before resize: {aud_real_init.shape}")
                        
                        # Handle different tensor formats
                        if aud_real_init.dim() == 3:  # (B, H, W) -> (B, 1, H, W)
                            aud_real_init = aud_real_init.unsqueeze(1)
                        elif aud_real_init.dim() == 2:  # (H, W) -> (1, 1, H, W)
                            aud_real_init = aud_real_init.unsqueeze(0).unsqueeze(0)
                        
                        # Resize to ImageBind standard dimensions
