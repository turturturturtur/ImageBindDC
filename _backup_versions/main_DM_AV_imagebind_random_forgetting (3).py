...
def run_forgetting_analysis(model, full_dataset, device, analysis_epochs=10, lr=0.01):
    ...
    def collate_fn_av(batch):
        ...
        # 先将样本列表堆叠成一个批次
        stacked_audios = torch.stack(audios) # 预期形状: (B, 1, 128, 204)
        stacked_frames = torch.stack(frames) # 预期形状: (B, 3, H, W)

        # ✨ 关键修正：将音频和视频都缩放到 224x224 的方形尺寸
        square_size = (224, 224)
        resized_audios = F.interpolate(stacked_audios, size=square_size, mode='bilinear', align_corners=False)
        resized_frames = F.interpolate(stacked_frames, size=square_size, mode='bilinear', align_corners=False)
        if torch.rand(1) < 0.1: # 大约10%的概率打印
             print(f"\n[DEBUG CHECK] Resized audio shape: {resized_audios.shape}, Resized frame shape: {resized_frames.shape}\n")


        inputs = {
            'audio': resized_audios,
            'frame': resized_frames
        }
    ...
    # 在创建 DataLoader 时，使用我们刚刚定义的 collate_fn
    loader = torch.utils.data.DataLoader(full_dataset, 
                                       batch_size=128, 
                                       shuffle=False, 
                                       num_workers=4,
                                       collate_fn=collate_fn_av)
    ...
    for epoch in range(analysis_epochs):
        ...
        for inputs, targets in pbar:
            ...
            # ✨ 修改二：使用 **inputs 将字典解包成关键字参数来调用模型
...
def main(args):
    ...
    for exp in range(args.num_exp):
        ...
        if args.selection_method == 'forgetting':
            # 1. 初始化一个临时模型用于分析
            print("Initializing a temporary model for Forgetting analysis...")
            # temp_nets, _ = get_network_imagebind(args) 
            # temp_model = temp_nets['net']
            _, temp_model = get_network_imagebind(args)
            temp_model = temp_model.to(args.device)
            # 2. 运行分析函数，获取所有样本的分数
            forgetting_scores = run_forgetting_analysis(temp_model, dst_train, args.device)
            
            # 3. 为每个类别，根据“忘记分数”对样本进行排序 (升序，忘记次数少的在前)
            forgetting_scores_sorted_indices = {c: [] for c in range(num_classes)}
            for c in range(num_classes):
                indices_of_class_c = np.array(indices_class[c])
                scores_of_class_c = forgetting_scores[indices_of_class_c]
                sorted_relative_indices = np.argsort(scores_of_class_c)
                sorted_global_indices = indices_of_class_c[sorted_relative_indices]
                forgetting_scores_sorted_indices[c] = sorted_global_indices.tolist()
