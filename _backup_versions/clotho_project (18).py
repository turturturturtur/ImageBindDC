...
class ImageBindAsRetriever(nn.Module):
    ...
    def forward(self, audio_paths, raw_captions):
        from nets.imagebind import data
        from nets.imagebind.models.imagebind_model import ModalityType

        with torch.no_grad():
            audio_inputs = {ModalityType.AUDIO: data.load_and_transform_audio_data(audio_paths, self.device)}
            text_inputs = {ModalityType.TEXT: data.load_and_transform_text(raw_captions, self.device)}
            audio_feats = self.imagebind(audio_inputs)[ModalityType.AUDIO]
            text_feats = self.imagebind(text_inputs)[ModalityType.TEXT]

        audio_feat = F.normalize(self.audio_head(audio_feats), dim=-1)
        text_feat = F.normalize(self.text_head(text_feats), dim=-1)
...
class RetrievalTrainer:
    ...
    def train(self, syn_dataset, input_dim, args, feature_dim=512):
        print(f"\n🎯 训练学生模型... (epochs: {args.student_epochs}, lr: {args.student_lr})")

        dataloader = DataLoader(syn_dataset, batch_size=8, shuffle=True)

        # ✅ 直接替换：当 input_dim == 1024（即 ImageBind 特征）时，使用 ImageBindAsRetriever
        if input_dim == 1024:
            from your_module import ImageBindAsRetriever  # 替换成你放的位置
            model = ImageBindAsRetriever(feature_dim=feature_dim, device=self.device)
            use_raw_input = True
        else:
        ...
        for epoch in range(args.student_epochs):
            ...
            for batch in dataloader:
                if use_raw_input:
                    ...
                    audio_feat, text_feat = model(audio_paths, raw_captions)
                else:
...
class StandardEvaluator:
    ...
    def evaluate(self,
                 model,
                 real_audio_features=None,
                 real_text_features=None,
                 real_audio_paths=None,
                 real_raw_captions=None):
        """
        标准 Recall@K 评估。
        支持两种模型：
        1. ImageBindAsRetriever：需要 real_audio_paths & real_raw_captions
        2. RetrievalModel：需要 real_audio_features & real_text_features
        """
        ...
        with torch.no_grad():
            # ---------- 统一获取特征 ----------
            if isinstance(model, ImageBindAsRetriever):
                # 使用 ImageBindAsRetriever
                if real_audio_paths is None or real_raw_captions is None:
                    raise ValueError("使用 ImageBindAsRetriever 时必须提供 real_audio_paths 和 real_raw_captions")
                audio_feat, text_feat = model(real_audio_paths, real_raw_captions)
            else:
                # 使用 RetrievalModel
                if real_audio_features is None or real_text_features is None:
                    raise ValueError("使用 RetrievalModel 时必须提供 real_audio_features 和 real_text_features")
...
class ClothoExperiment:
    ...
    def run_complete_experiment(self, args):
        ...
        for name, synthetic_data, real_eval_features in distilled_datasets:
            ...
            trained_model = self.trainer.train(syn_dataset, input_dim, args, feature_dim=512)
            
            # 关键：使用从未见过的评估集特征进行评估
            eval_audio_features, eval_text_features = real_eval_features
            if isinstance(trained_model, ImageBindAsRetriever):
                # ✅ 使用原始路径和文本
                eval_results = self.evaluator.evaluate(
                    trained_model,
                    real_audio_paths=eval_dataloader.dataset.captions_df['file_name'].tolist(),
                    real_raw_captions=eval_dataloader.dataset.captions_df['caption_1'].tolist()
                )
            else:
                # ✅ 使用特征
