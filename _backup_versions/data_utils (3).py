...
def get_train_dataset_origin(dataset, args):
    ...
    if dataset == 'VGG_subset':
        ...
        # Resize audio to ImageBind standard dimensions 128x204
        import torch.nn.functional as F
        if aud_train.dim() == 3:  # (B, H, W)
            aud_train = aud_train.unsqueeze(1)  # (B, 1, H, W)
        ...
        aud_train   = [p for p in aud_train   if os.path.exists(p) and os.path.getsize(p) > 0]
        # 2. 过滤掉不存在或 0 字节的图像
        images_train = [p for p in images_train if os.path.exists(p) and os.path.getsize(p) > 0]
        # 3. 保持音频、图像、标签一一对应
        valid_idx = [i for i, (a, v) in enumerate(zip(aud_train, images_train)) if a and v]
        aud_train   = [aud_train[i]   for i in valid_idx]
        images_train = [images_train[i] for i in valid_idx]
        labels_train = [labels_train[i] for i in valid_idx]
        aud_train = [p for p in aud_train if os.path.exists(p) and os.path.getsize(p) > 0]
...
def get_test_dataset_origin(dataset, args):
    ...
    if dataset == 'VGG_subset':
        ...
        # Resize audio to ImageBind standard dimensions 128x204
        import torch.nn.functional as F
        if aud_test.dim() == 3:  # (B, H, W)
            aud_test = aud_test.unsqueeze(1)  # (B, 1, H, W)
        ...
        # for c in range(channel[1]):
        #     images_test[:, c] = (images_test[:, c] - mean[c]) / std[c]
        # Convert lists to tensors and apply preprocessing
        aud_test = torch.stack(aud_test).detach().float()
        images_test = torch.stack(images_test).detach().float() / 255.0
        labels_test = torch.tensor(labels_test).detach().long()
        for c in range(channel[1]):
            images_test[:, c] = (images_test[:, c] - mean[c]) / std[c]
...
class CombTensorDataset_origin(Dataset):
    ...
    def __getitem__(self, index):
        label = self.labels[index]
        audio = self.audio_data[index] if self.args.input_modality in ['a', 'av'] else None
        frame = self.frame_data[index] if self.args.input_modality in ['v', 'av'] else None
