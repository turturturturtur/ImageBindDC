...
class PairedAVDataset(torch.utils.data.Dataset):
    """
    一个包装类，将包含音视频数据的字典转换成可通过索引访问的 Dataset.
    """
    def __init__(self, av_dataset_dict):
        self.vision_dataset = av_dataset_dict[ModalityType.VISION]
        self.audio_dataset = av_dataset_dict[ModalityType.AUDIO]
        self.classes = self.vision_dataset.classes
        # 安全检查，确保两个数据集长度一致
        assert len(self.vision_dataset) == len(self.audio_dataset), \
            "视觉和音频数据集的长度不匹配!"
    ...
    def __getitem__(self, index):
        # 根据索引分别从音视频数据集中取出数据
        vision_data, vision_label = self.vision_dataset[index]
        audio_data, audio_label = self.audio_dataset[index]
        
        # 再次检查，确保同个索引的标签是一致的
        assert vision_label == audio_label, f"索引 {index} 处的标签不匹配!"
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        if "subset" in checkpoint.keys():
            ...
            if args.module != "av":
                ...
                # 2. ✨ 将数据字典用我们新的包装类 PairedAVDataset 包裹起来
                paired_dataset = PairedAVDataset(dst_train_av_dict)
                
                # 3. 将包装好的、可通过索引访问的 paired_dataset 传给筛选方法
                print("Performing unified multi-modal selection...")
                method = methods.__dict__[args.selection](paired_dataset, args, args.fraction, args.seed, **selection_args)
                subset = method.select()
