...
if __name__ == "__main__":
    ...
    # ### MODIFICATION START ###
    # 使用您服务器上的绝对路径
    BASE_CLOTHO_PATH = '/autodl-tmp/audio-visual-mater/OpenDataLab___Clotho/raw/Clotho/'
    
    # development音频文件位于您提供的路径
    AUDIO_DIR = os.path.join(BASE_CLOTHO_PATH, 'development') 
    
    # captions文件通常与development目录同级
    CAPTIONS_FILE = os.path.join(BASE_CLOTHO_PATH, 'clotho_captions_development.csv')
    # ### MODIFICATION END ###
    ...
    if not os.path.exists(AUDIO_DIR) or not os.path.exists(CAPTIONS_FILE):
         ...
         # 运行实验
         experiment.run_complete_experiment(num_real_samples=2893, num_syn_samples=20) # 建议使用全量数据