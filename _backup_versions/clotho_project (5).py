...
# 这个操作能让Python找到 nets 和 utils 文件夹
# 请根据您的实际文件结构调整此路径
...
class RawClothoDataset(Dataset):
    ...
    # 在 RawClothoDataset 类中
    def __getitem__(self, idx):
        row = self.captions_df.iloc[idx]
        audio_path = os.path.join(self.audio_dir, row['file_name'])
        
        # 为 ConvNetGRU 准备梅尔频谱图
        y, _ = librosa.load(audio_path, sr=self.sr)
        mel_spec = librosa.feature.melspectrogram(y=y, sr=self.sr, n_mels=self.n_mels)
        audio_mel_tensor = torch.tensor(librosa.power_to_db(mel_spec), dtype=torch.float32).unsqueeze(0)
        
        # 为 ConvNetGRU 准备文本ID序列
    ...
    return {
        ...
        'audio_path': audio_path,       # 【新增】为ImageBind提供文件路径
        'raw_caption': caption_string   # 【新增】为ImageBind提供原始字符串
    }
...
def precompute_real_features(dataloader: DataLoader, feature_extractor, device: str) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    # 将模型移至指定设备并设置为评估模式
    if isinstance(feature_extractor, nn.Module): # 只有nn.Module才有.to和.eval
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="正在提取特征"):
            # 根据模型类型，从batch中获取不同格式的数据
            if isinstance(feature_extractor, ImageBindExtractor):
                # 错误点修正：为ImageBind传递【文件路径列表】和【原始字符串列表】
                # 旧代码: audio_raw = batch['audio'].to(device)
                # 新代码: 直接使用 batch['audio_path']
                audio_data = batch['audio_path']
                text_data = batch['raw_caption']
                
                audio_feats = feature_extractor.extract_audio_features(audio_data)
                text_feats = feature_extractor.extract_text_features(text_data)

            else: # 适用于 ConvNetGRU 等双塔模型
                # 错误点修正：为ConvNetGRU传递【频谱图张量】和【ID序列张量】
                # 旧代码: audio_raw = batch['audio'].to(device)
                # 新代码: audio_data = batch['audio_mel'].to(device)
...
class ImageBindExtractor:
    ...
    def __init__(self, pretrained=False, device='cpu'):
        ...
        if pretrained:
            ...
            # 创建一个临时的、满足 get_network_imagebind 函数需求的参数对象
            class MockArgs:
                def __init__(self):
                    self.arch_frame = 'imagebind' # 模仿您项目中可能的默认值
                    self.arch_classifier = 'ensemble'
                    self.cls_num = 10 # 占位符，因为我们不需要分类器
            ...
            net_imagebind, _ = nets # 我们只需要主干网络，忽略分类器
            
            self.model = net_imagebind
            self.model.to(self.device)
            self.model.eval()

            # 获取embedding函数，处理DataParallel的情况
    ...
    def extract_audio_features(self, audio_path_list: List[str]) -> torch.Tensor:
        ...
        # 使用官方的data工具进行预处理
        inputs = {
            ModalityType.AUDIO: data.load_and_transform_audio(audio_path_list, self.device),
        }
        with torch.no_grad():
    ...
    def extract_text_features(self, raw_caption_list: List[str]) -> torch.Tensor:
        ...
        # 使用官方的data工具进行预处理
        inputs = {
            ModalityType.TEXT: data.load_and_transform_text(raw_caption_list, self.device),
        }
        with torch.no_grad():
            embeddings = self.embed_func(inputs)
...
def run_complete_experiment(self, args):
        ...
        result1 = self.distiller.distill(real_convnet_audio, real_convnet_text, method='avdd', num_syn_samples=args.ipc, iterations=args.distill_iter)
        result2 = self.distiller.distill(real_imagebind_audio, real_imagebind_text, method='imagebind_dc', num_syn_samples=args.ipc, iterations=args.distill_iter)
        result3 = self.distiller.distill(real_imagebind_audio, real_imagebind_text, method='avdd', num_syn_samples=args.ipc, iterations=args.distill_iter)
        result4 = self.distiller.distill(real_convnet_audio, real_convnet_text, method='imagebind_dc', num_syn_samples=args.ipc, iterations=args.distill_iter)
        
        # ===================== 阶段二 & 三：训练并【在评估集上】评估检索模型 =====================
        print("\n🔬 阶段二 & 三：在合成数据上训练并使用【评估集】评估...")
        
        distilled_datasets = [
            ("AVDD_ConvNet", result1, (eval_convnet_audio, eval_convnet_text)),
            ("ImageBindDC_ImageBind", result2, (eval_imagebind_audio, eval_imagebind_text)),
            ("AVDD_ImageBind", result3, (eval_imagebind_audio, eval_imagebind_text)),
            ("ImageBindDC_ConvNet", result4, (eval_convnet_audio, eval_convnet_text))
        ]
        ...
        for name, synthetic_data, real_eval_features in distilled_datasets:
            ...
            trained_model = self.trainer.train(syn_dataset, input_dim=input_dim, feature_dim=512, epochs=args.student_epochs)
        ...
        # 使用 args 中的 seed 和 ipc 命名文件，更具可读性
...
if __name__ == "__main__":
    ...
    # --- 路径与数据参数 ---
    parser.add_argument('--base_path', type=str, default='/autodl-tmp/audio-visual-mater/OpenDataLab___Clotho/raw/Clotho/', help='Clotho数据集的根目录')
    ...
    parser.add_argument('--distill_iter', type=int, default=1000, help='蒸馏优化的迭代次数')
    parser.add_argument('--student_epochs', type=int, default=100, help='训练学生检索模型的轮数')
    parser.add_argument('--distill_lr', type=float, default=0.01, help='蒸馏阶段优化器的学习率')
