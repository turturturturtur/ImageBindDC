...
def visualize_embedding_space_tsne(audio_features, text_features, file_path, num_samples=500):
    """
    使用t-SNE可视化音文联合嵌入空间，并将图片保存到文件。
    """
    print(f"\n🎨 正在生成t-SNE嵌入空间图 (采样数: {num_samples})，并保存至 {file_path}...")

    # 从特征中随机采样一部分，以加速t-SNE计算
    ...
    audio_features_cpu = audio_features.detach().cpu().numpy()
    text_features_cpu = text_features.detach().cpu().numpy()

    all_features = np.vstack([audio_features_cpu, text_features_cpu])
    labels = np.array(['Audio'] * len(audio_features) + ['Text'] * len(text_features))

    # 运行t-SNE
    # 【关键修正】将已过时的 n_iter 参数替换为 max_iter
    ...
    plt.title('t-SNE Visualization of Audio-Text Joint Embedding Space', fontsize=16)
...
class ClothoExperiment:
    ...
    def run_complete_experiment(self, args):
        ...
        for name, synthetic_data, real_eval_features in distilled_datasets:
            ...
            tsne_save_path = f'tsne_{name}_seed{args.seed}.png'
            visualize_embedding_space_tsne(
                projected_audio, 
                projected_text, 
                file_path=tsne_save_path
            )
