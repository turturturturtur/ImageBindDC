...
class ClothoExperiment:
    ...
    def run_complete_experiment(self, args):
        ...
        for name, synthetic_data, real_eval_features in distilled_datasets:
            ...
            # 根据蒸馏组合的名称选择学生模型类型
            if "ImageBind" in name: # 如果蒸馏发生在 ImageBind 特征空间，则使用 ImageBindAsRetriever
                student_model_type = 'ImageBindAsRetriever'
            else: # 否则，如果蒸馏发生在 ConvNet 特征空间，则使用 RetrievalModel
            ...
            trained_model = self.trainer.train(
                ...
                args, 
                feature_dim=512,
                model_type=student_model_type # <-- 传入模型类型
            )
