...
class EfficientDataset(Dataset):
    """
    一个从独立样本文件按需加载数据的PyTorch Dataset类。
    对于超大规模数据集，这种方式内存效率极高。
    """
    def __init__(self, root_dir, transform=None):
        """
        Args:
            root_dir (string): 包含所有类别子文件夹 ('class_0', 'class_1'...) 的根目录。
            transform (callable, optional): 可选的数据转换操作。
        """
        self.root_dir = root_dir
        self.transform = transform
        
        # 扫描目录以找到所有独立的样本文件路径并存储。
        # 这个步骤非常快，且几乎不消耗内存。
        self.sample_files = sorted(glob.glob(os.path.join(root_dir, 'class_*', '*.pt')))
        
        if not self.sample_files:
            raise RuntimeError(f"在 {root_dir} 的子目录中没有找到任何 .pt 文件")
    ...
    def __getitem__(self, idx):
        ...
        # 按需加载这一个样本的 .pt 文件。
        try:
            sample_data = torch.load(file_path, map_location='cpu') # 确保加载到CPU
        except Exception as e:
            print(f"加载文件时出错: {file_path}。错误: {e}")
            # 返回一个虚拟样本以防程序崩溃，您也可以根据需要调整错误处理方式。
            # 虚拟数据的形状应与您的真实数据形状一致。
            return {
                'audio': torch.zeros(1, 128, 56), # 示例形状，请根据您的数据调整
                'frame': torch.zeros(3, 224, 224), # 示例形状
...
def get_test_dataset_origin(dataset, args):
    ...
    if dataset == 'VGG_subset':
        ...
        root_dir = '/root/autodl-tmp/audio-visual-master/data/test_data/vggsound_test'
        dst_test = EfficientDataset(root_dir=root_dir)
