...
def main(args):
    ...
    # Ensure square dimensions for ImageBind compatibility
    square_size = min(im_size[1][0], im_size[1][1])
    square_size = max(square_size, 224)  # Minimum size for ImageBind
    im_size = list(im_size)
    im_size[1] = (224, 224)  # Force ImageBind standard size
    im_size[0] = (224, 224)  # Force ImageBind standard size for audio
    ...
    for exp in range(args.num_exp):
        ...
        if args.input_modality == 'a' or args.input_modality == 'av':
           # Ensure audio dimensions are compatible with ImageBind (1, 128, 128)
           aud_syn = torch.randn(size=(num_classes*args.ipc, channel[0], 1, 128, 128), dtype=torch.float, requires_grad=True, device=args.device)
        
        if args.input_modality == 'v' or args.input_modality == 'av':
            # Ensure image dimensions are compatible with ImageBind
        ...
        for c in range(num_classes):
            if not args.idm_aug:
                ...
                if aud_real_init is not None and (args.input_modality == 'a' or args.input_modality == 'av'):
                    # 1. 确保有通道维度, 变为 (B, C, H, W)
                    if aud_real_init.dim() == 3: # Shape: (ipc, H, W)
                        aud_real_init = aud_real_init.unsqueeze(1) # Becomes (ipc, 1, H, W)
                    elif aud_real_init.dim() == 4: # Shape: (ipc, C, H, W)
                        pass  # Already correct
                    else:
                        # Handle unexpected dimensions
                        aud_real_init = aud_real_init.view(-1, 1, 128, 56)
                    
                    # 2. 使用 F.interpolate 将其缩放到目标尺寸 (128, 128)
                    import torch.nn.functional as F
                    aud_real_init_resized = F.interpolate(aud_real_init, size=(128, 128), mode='bilinear', align_corners=False)

                    # 3. 在赋值前添加 ImageBind 需要的额外维度，变为 (B, C, 1, H, W)
                    aud_real_init_resized = aud_real_init_resized.unsqueeze(2)
                ...
                if img_real_init is not None and (args.input_modality == 'v' or args.input_modality == 'av'):
                    # 如果视频尺寸不匹配，也进行缩放
                    if img_real_init.dim() == 3: # Shape: (ipc, H, W)
                        img_real_init = img_real_init.unsqueeze(1)
        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            for c in range(num_classes):
                ...
                # Ensure all inputs are resized to 224x224 for ImageBind
                img_real = resize_to_square(img_real, 224) if img_real is not None else None
                aud_real = resize_to_square(aud_real, 224) if aud_real is not None else None
                ...
                curr_aud_syn = resize_to_square(curr_aud_syn, 224) if curr_aud_syn is not None else None
                img_real = resize_to_square(img_real, 224) if img_real is not None else None
                aud_real = resize_to_square(aud_real, 224) if aud_real is not None else None
                ...
                embd_img_syn = embeddings_syn.get(ModalityType.VISION, torch.zeros(1, 768).to(args.device)).detach()
                embd_aud_syn = embeddings_syn.get(ModalityType.AUDIO, torch.zeros(1, 768).to(args.device)).detach()
