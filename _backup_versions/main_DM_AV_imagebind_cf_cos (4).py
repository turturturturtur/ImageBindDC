        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            for c in range(num_classes):
                ...
                if args.input_modality == 'av':
                    ...
                    if it == 0:
                        print(f"\n--- Iteration 0, Class {c} ---")
                        print(f"  Loss Base_DM (weighted): {loss_base_weighted.item():.6f} (weight: {args.lam_base})")
                        print(f"  Loss ICM (weighted):     {loss_icm_weighted.item():.6f} (weight: {args.lam_icm})")
                        print(f"  Loss CGM (weighted):     {loss_cgm_weighted.item():.6f} (weight: {args.lam_cgm})")
                        print(f"--------------------------")
