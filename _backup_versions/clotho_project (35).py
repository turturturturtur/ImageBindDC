    ...
    def train(self, syn_dataset, real_feature_dataset, args, student_model_type, feature_dim=512):
        ...
        if student_model_type == 'convnet':
            ...
            optimizer = torch.optim.Adam(
                ...
                lr=args.student_lr,
                weight_decay=1e-4 # 添加weight_decay
            )
...
class ClothoExperiment:
    ...
    def run_complete_experiment(self, args):
        """
        执行最终版实验（仅ImageBind空间），对比 DM, AVDD, ImageBindDC。
        """
        ...
        # 方法三: DM (作为AVDD的简化基线)
        print("\n--- (3/3) DM on ImageBind ---")
        # 通过将AVDD的交叉项权重设为0，我们得到一个纯粹的分布匹配(DM)基线
        import copy
        ...
        #     # 所有实验都使用 'imagebind' 类型的学生模型 (StudentRetriever)
        ...
        # ===================== Whole-Data 训练（替换原阶段二&三） =====================
        print("\n🔬 Whole-Data 训练（跳过蒸馏，直接使用全部真实数据）...")
        
        # 用完整真实数据训练学生模型
        real_whole_dataset = SyntheticDataset(real_imagebind_audio, real_imagebind_text)
        
        trained_model = self.trainer.train(
            syn_dataset=real_whole_dataset,
            real_feature_dataset=real_whole_dataset,  
            args=args,
            student_model_type='imagebind',
            feature_dim=512
        )
        ...
        eval_results = self.evaluator.evaluate(
            trained_model,
            eval_imagebind_audio,
            eval_imagebind_text
        )
        
        # 打印并保存结果
        final_results = {"WholeData_ImageBind": eval_results}
        print("\n" + "="*80)
        print("📊 Whole-Data 评估结果")
        print("="*80)
        print(f"{'方法':<25} {'R@1_A2T':<12} {'R@1_T2A':<12} "
              f"{'R@5_A2T':<12} {'R@5_T2A':<12} {'R@10_A2T':<12} {'R@10_T2A':<12}")
        print("-"*90)
        print(f"{'WholeData_ImageBind':<25} "
              f"{eval_results.get('R@1_a2t', 0.0):<12.4f} {eval_results.get('R@1_t2a', 0.0):<12.4f} "
              f"{eval_results.get('R@5_a2t', 0.0):<12.4f} {eval_results.get('R@5_t2a', 0.0):<12.4f} "
              f"{eval_results.get('R@10_a2t', 0.0):<12.4f} {eval_results.get('R@10_t2a', 0.0):<12.4f}")
        # # ===================== 阶段四：结果汇总 =====================
        # print("\n" + "=" * 80)
        # print("📊 最终对比矩阵")
        # print("=" * 80)
