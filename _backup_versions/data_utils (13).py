...
class EfficientDataset(Dataset):
    """
    增强版的 EfficientDataset，支持通过原始索引进行访问，以兼容预计算的 Herding 文件。
    """
    def __init__(self, root_dir, transform=None):
        self.root_dir = root_dir
        self.transform = transform
        
        print(f"正在为 {root_dir} 构建索引...")
        all_files = glob.glob(os.path.join(root_dir, 'class_*', '*.pt'))
        
        # 创建一个从“原始索引”到“文件路径”的映射
        self.original_index_map = {}
        for f_path in all_files:
            try:
                # 从文件名 '..._sample_123.pt' 中提取出原始索引 123
                original_idx = int(os.path.basename(f_path).split('_')[-1].split('.')[0])
                self.original_index_map[original_idx] = f_path
            except (ValueError, IndexError):
                continue # 忽略无法解析的文件名
        
        # 创建一个用于常规遍历的、排序后的原始索引列表
        self.sorted_original_indices = sorted(self.original_index_map.keys())

        if not self.original_index_map:
            raise RuntimeError(f"在 {root_dir} 的子目录中没有找到任何符合命名规则的 .pt 文件")
        
        print(f"索引构建完成，共找到 {len(self.sorted_original_indices)} 个样本。")

    def __len__(self):
        # [需要修改] 长度是有效样本的数量
        return len(self.sorted_original_indices)

    def __getitem__(self, idx):
        # [需要修改] idx 是 DataLoader 提供的常规索引 (0, 1, 2, ...)
        # 1. 将常规索引转换为原始索引
        original_idx = self.sorted_original_indices[idx]
        
        # 2. 使用原始索引从 map 中找到文件路径
        file_path = self.original_index_map[original_idx]
        
        try:
            sample_data = torch.load(file_path, map_location='cpu')
        except Exception as e:
            print(f"加载文件时出错: {file_path}。错误: {e}")
            return {'audio': torch.zeros(1, 128, 56), 'frame': torch.zeros(3, 224, 224), 'label': torch.tensor(-1, dtype=torch.long)}
...
#     elif dataset == 'vggsound_train': 
#         # 路径指向您刚刚成功生成的文件夹
#         root_dir = '/root/autodl-tmp/audio-visual-master/data/train_data/vggsound_train'
#         dst_train = EfficientDataset(root_dir=root_dir)

#         # 您可能需要通过扫描子文件夹名称来确定类别总数
#         import glob
#         class_folders = glob.glob(os.path.join(root_dir, 'class_*'))
#         num_classes = len(class_folders)

#         # ... 根据需要返回 channel, im_size, mean, std 等信息 ...
#         # 这里的 channel 和 im_size 可以是基于您数据集知识的固定值
#         channel = [1, 3] # 示例: [音频通道, 视频通道]
#         im_size = [(128, 56), (224, 224)] # 示例: [音频尺寸, 视频尺寸]
#         mean = [0.485, 0.456, 0.406] # 示例
#         std = [0.229, 0.224, 0.225]  # 示例
...
def get_train_dataset_origin(dataset, args):
    """
    专门负责加载【训练】数据集。
    所有分支只负责准备数据，最后由统一的 return 语句返回。
    """
    
    # --- 为所有分支定义通用变量 ---
    mean, std, num_classes, channel, im_size, dst_train = [None]*6

    if dataset == 'VGG_subset':
        data = torch.load('data/train_data/vgg_subset_train.pt', map_location='cpu')
        mean, std, num_classes = [0.425, 0.396, 0.370], [0.229, 0.224, 0.221], 10
        channel, im_size = [1, 3], [(128, 204), (224, 224)]
        
        aud_train = data['audio_train'].detach().float()
        if aud_train.shape[-2:] != (128, 204):
             aud_train = F.interpolate(aud_train, size=(128, 204), mode='bilinear', align_corners=False)
    ...
    elif dataset == 'vggsound_split': 
        root_dir = '/root/autodl-tmp/audio-visual-master/data/train_data/vggsound_split'
        dst_train = EfficientDataset(root_dir=root_dir)
        ...
        # 定义数据集的元数据
        channel = [1, 3]
        im_size = [(128, 56), (224, 224)]
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]
    
    # 您可以按照上面的模式继续添加其他的 elif 分支
    # elif dataset == 'AVE':
    #     ...
    
    else:
        exit('unknown training dataset: %s' % dataset)
    
    # 所有分支都走到这里，使用统一的 return 语句
    return channel, im_size, num_classes, mean, std, dst_train
...
#         # 4. Dataset和Loader
#         dst_test = CombTensorDataset_origin(aud_test, images_test, labels_test, args)
#     elif dataset == 'vggsound_test':
#         print("Loading split testing data from vggsound_test...")
#         root_dir = '/root/autodl-tmp/audio-visual-master/data/train_data/vggsound_test'
#         dst_test = EfficientDataset(root_dir=root_dir)
...
#         # 测试函数期望返回 testloader
#         testloader = torch.utils.data.DataLoader(dst_test, batch_size=args.batch_syn, shuffle=False, num_workers=args.num_workers)
#     else:
#         exit('unknown dataset: %s'%dataset)
...
def get_test_dataset_origin(dataset, args):
    ...
    # --- 为所有分支定义通用变量 ---
    mean, std, num_classes, channel, im_size, dst_test = [None]*6
    ...
    if dataset == 'VGG_subset':
        ...
        aud_test = data['audio_test'].detach().float()
        if aud_test.shape[-2:] != (128, 204):
            aud_test = F.interpolate(aud_test, size=(128, 204), mode='bilinear', align_corners=False)
        ...
        root_dir = '/root/autodl-tmp/audio-visual-master/data/test_data/vggsound_split'
        dst_test = EfficientDataset(root_dir=root_dir)
    ...
    # ... 您可以按此模式添加其他数据集的 elif 分支 ...

    else:
        exit('unknown testing dataset: %s'%dataset)
    
    # [修正] 将 DataLoader 的创建和 return 语句统一放在函数末尾
    # 这样所有数据集都会使用相同的 batch_size 和 num_workers
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=args.batch_syn, shuffle=False, num_workers=args.num_workers)
...
def get_herd_path(dataset):
    if dataset == 'VGG_subset':
        ...
        # 指向您已有的、为 vggsound 生成的 herding 文件
        return 'data/herding_data/VGG_herd_idx_dict.pkl' 
    else:
        exit('unknown dataset: %s'%dataset)
