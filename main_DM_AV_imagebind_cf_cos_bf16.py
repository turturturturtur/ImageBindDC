...
def main(args):
    ...
    for exp in range(args.num_exp):
        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            # 1. 在每个迭代(it)的开始，将 loss_avg 初始化为 0，并将优化器梯度清零
            loss_avg = 0
            ...
            for c in range(num_classes):
                ...
                inputs_syn = {
                    ModalityType.VISION: curr_img_syn,
                ...
                # 3. 【重要】累加当前类别的损失值，用于日志记录
                loss_avg += loss_c.item()
                
                # 4. 【重要】计算当前类别的梯度，并将其累加到 .grad 属性上
                loss_c.backward()

            # 5. 【重要】在所有类别的梯度都计算并累加完毕后，执行一次参数更新
            optimizer_comb.step()

            # 6. 【重要】计算本次迭代的平均损失，用于打印
