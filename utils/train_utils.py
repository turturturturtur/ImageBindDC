...
def get_network_imagebind(args, pretrained=True):
    ...
    # 确保分类器输入维度正确（内存规范6）
    # 音频和视觉特征各1024维，拼接后为2048维
    net_classifier = builder.build_classifier(
        arch=args.arch_classifier,
        cls_num=args.cls_num,
        weights=args.weights_classifier,
        input_modality=args.input_modality,
        input_size=2048)  # 明确标注维度来源

    # 确保nets元组包含有效对象
    if net_imagebind is None or net_classifier is None:
        raise ValueError("Failed to build network components")
...
def get_network_imagebind(args, pretrained=True):
    ...
    net_classifier = builder.build_classifier(
        ...
        weights=args.weights_classifier,
        input_modality=args.input_modality,
        input_size=2048)  # 关键参数修改
        
    # 添加维度校验钩子函数（内存规范6）
    def check_input_dim(module, input):
        assert input[0].size(1) == 2048, f"分类器输入维度错误: {input[0].size(1)}"
