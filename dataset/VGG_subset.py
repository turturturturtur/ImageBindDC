...
class VGGSubsetDataset(Dataset):
    """
    一个标准的 PyTorch Dataset，用于处理 VGG_subset 数据集。
...
@DATASET.register('VGG_subset')
class VGGSubsetBuilder(BaseDataset):
    """
    VGG_subset 数据集的构建器 (Builder)。

    这个类被注册到全局的 DATASETS 注册表中，并由工厂函数调用。
    它的 `build` 方法负责创建并返回一个功能完整的 VGGSubsetDataset 实例。
    """
    def __init__(self, config_path: str = "config/dataset/VGG_subset.yaml"):
        """
        初始化构建器，主要是加载和解析配置文件。
        这个阶段不执行任何耗时的数据加载操作。
    ...
    def build(self, mode: str = 'train', transform: Callable=None) -> VGGSubsetDataset:
        """
        构建并返回一个 VGGSubsetDataset 实例。
        ...
        Returns:
            VGGSubsetDataset: 一个配置好的、包含数据的 VGGSubsetDataset 实例。
        """
        ...
        # 4. 根据 input_modality 决定要传递给 VGGSubsetDataset 的数据
        ...
        # 5. 创建并返回最终的 VGGSubsetDataset 实例
        dataset_instance = VGGSubsetDataset(
