...
def main(args):
    ...
    # 确保音频维度兼容性，支持(1024, 2048)等变体
    if len(im_size[0]) == 2:
        aud_h, aud_w = im_size[0]
        # 如果检测到1024x2048，调整为合适的维度
        if aud_h == 1024 and aud_w == 2048:
            im_size[0] = (1024, 2048)  # 保持原始维度
            print(f'Detected audio dimension: {im_size[0]} - using direct mapping')
        elif aud_h == 128 and aud_w in [56, 204]:
            print(f'Using standard audio dimension: {im_size[0]}')
        else:
            print(f'Using custom audio dimension: {im_size[0]}')
...
if __name__ == '__main__':
    ...
    # --- 核心训练参数 ---
    parser.add_argument('--dataset', type=str, default='AVE', help='dataset')
    ...
    parser.add_argument('--wandb_disable', action='store_false', help='use to enable wandb logging')
