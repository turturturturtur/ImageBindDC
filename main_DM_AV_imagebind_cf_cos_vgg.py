                ...
                if args.input_modality == 'a' or args.input_modality == 'av':
                    ...
                    if aud_real_init is not None:
                        ...
                        if aud_real_init.dim() == 3:
                            aud_real_init = aud_real_init.unsqueeze(1)  # (ipc, 1, H, W)
                    
                        # 统一 resize 到 ImageBind 标准尺寸 (128, 204)
                        aud_real_init = F.interpolate(
                            aud_real_init, size=(128, 204), mode='bilinear', align_corners=False
                        )  # (ipc, 1, 128, 204)
                    
                        # 变成 5D: (ipc, 1, 1, 128, 204) 与 aud_syn 对齐
                        aud_real_init = aud_real_init.unsqueeze(2)
        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            for c in range(num_classes):
                ...
                if args.input_modality == 'av':
                    ...
                    loss_base_weighted = args.lam_base * (loss_base_aud + loss_base_vis) # 应用权重
                    ...
                    loss_icm_weighted = args.lam_icm * torch.mean(1 - cos_sim) # 应用权重
                    ...
                    loss_cgm_weighted = args.lam_cgm * torch.mean(1 - cross_cos_lsss) # 应用权重

                    # 在第一个迭代时打印各分量，方便调参
                    if it == 0 or it==1:
                        tqdm.write(f"\n--- Iteration {it}, Class {c} ---")
                        tqdm.write(f"  Loss Base_DM (weighted): {loss_base_weighted.item():.6f} (weight: {args.lam_base})")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_syn_aud', type=float, default=0.1, help='learning rate for updating synthetic audio specs')
