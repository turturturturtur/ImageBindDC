...
from utils.data_utils import  get_herd_path, \
...
from utils.data_utils_sound import get_test_dataset_origin, get_train_dataset_origin
...
def main(args):
    ...
    # print(f"✅ 数据集加载验证:")
    # # 打印训练集总样本数
    # print(f"  - 训练集总样本数 (len(dst_train)): {len(dst_train)}") 
    # # 打印测试集总样本数
    # print(f"  - 测试集总样本数 (len(testloader.dataset)): {len(testloader.dataset)}")
    # print(f"  - 识别出的总类别数 (num_classes): {num_classes}")
    # print("-" * 50)
        ...
        for it in tqdm(range(args.Iteration+1)):
            ...
            for c in range(num_classes):
                ...
                with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                    inputs_real = {ModalityType.VISION: img_real, ModalityType.AUDIO: aud_real}
                    ...
                    inputs_syn = {ModalityType.VISION: curr_img_syn, ModalityType.AUDIO: curr_aud_syn}
                ...
                #【修复关键点 1】将当前类的 loss 值累加到 loss_avg，用于后续打印
                loss_avg += loss_c.item()
                
                # 2. 对每个类的 loss 进行反向传播，梯度会自动累积
                loss_c.backward()

            # 3. 在所有类别循环结束后，统一更新一次参数
            optimizer_comb.step()
            
            # 4. 统一更新一次学习率
            
            
            #【修复关键点 2】计算本次迭代的平均 loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_syn_aud', type=float, default=1, help='learning rate for updating synthetic audio specs')
    parser.add_argument('--lr_syn_img', type=float, default=1, help='learning rate for updating synthetic image')
